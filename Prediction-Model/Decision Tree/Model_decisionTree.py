'''
This is model for decision tree, it read samples from a directory named Commplete Datasets(from parent directory) and output classification report, confusion matrix, accuracy, decision tree and map from number to label name.

If you want to run this file with your own scikit-learn package, you need to make some changes in scikit-learn package(However, we have already provided a modified package just in the current directory, so you can run this file directly if you have installed scikit-learn packpage(0.15.2 version)) and you need to install pydot to output visualized decision tree:
First, you need to modify fit function of pipeline.py as following:
    1)Adding parameter i_fold in the function header;
    2)Adding the following in after calling __pre_tranform__ function:
...
Second, you need to modify __pre_transform__ function of pipeline.py as following:
    ...
    2)Adding the following right after calling fit_transform, which will output map from vocabulary index to vocabulary:
